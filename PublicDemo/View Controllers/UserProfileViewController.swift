        ...
        if !(locationField.text?.isEmpty)! {
            ...
            guard let locationArray = locationField.text?.split(separator: ",").map({ String($0) }),
                ...
                let alert = UIAlertController.init(title: "Incorrect Location format", message: "Enter location as \"19,72\"", preferredStyle: .alert)
                    ...
                    alert.addAction(UIAlertAction.init(title: "Got It", style: .default, handler: { (action) in
...
extension UserProfileViewController {
    ...
    @IBAction func deleteCustomTapped(_ sender: UIButton) {
        ...
        alert.addAction(UIAlertAction.init(title: "Delete Single Key", style: .default, handler: { (action) in
            self.presentDeleteAlert(for: "Single")
        }))
        
        alert.addAction(UIAlertAction.init(title: "Delete Multiple Keys", style: .default, handler: { (action) in
...
extension UserProfileViewController {
    ...
    @IBAction func addCustomTapped(_ sender: UIButton) {
        ...
        alert.addAction(UIAlertAction.init(title: "String Value", style: .default, handler: { (action) in
            self.presentAlert(for: "String")
        }))
        
        alert.addAction(UIAlertAction.init(title: "Boolean Value", style: .default, handler: { (action) in
            self.presentAlert(for: "Boolean")
        }))
        
        alert.addAction(UIAlertAction.init(title: "Date Value", style: .default, handler: { (action) in
            self.presentAlert(for: "Date")
        }))
        
        alert.addAction(UIAlertAction.init(title: "Array Value", style: .default, handler: { (action) in
            self.presentAlert(for: "Array")
        }))
        
        alert.addAction(UIAlertAction.init(title: "Dictionary Value", style: .default, handler: { (action) in
